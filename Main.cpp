...
int F1(int i)
{
    ...
    for (j = 0; j < 6; j++)
        b[j] = X[i][j];
    // std::cout << "done 2: " << b[0] << ":" << b[5] << ":" << c;
    r = b[0] * 2 + b[5];
    c = 8 * b[1] + 4 * b[2] + 2 * b[3] + b[4];

    // std::cout << "done 2: " << i << ":" << r << ":" << c;
    if (i == 0)
        return S1[r][c];
    else if (i == 1)
        return S2[r][c];
...
void convertCharToBit(long int n)
{
...
void Encryption_r(int value, FILE *out)
{
    int cipher;
...
{ // destroy contents of these files (from previous runs, if any)
    ...
    else
    {
        ...
        char o;
        // RSA
        printf("\n\nDO YOU WANT TO encrypt the DES key using RSA(Y/N): ");
        scanf("%c", &o);
        if (o == 'y' || o == 'Y')
        {
            printf("\n\nENTER THE PUBLIC KEY(n,e) send from the receivers: ");
            scanf("%d", &n);
            scanf("%d", &e);
            int phi_n = phi(n);
            inp = fopen("key.txt", "r+");
            ...
            fclose(inp);
            fclose(out);
            printf("\n\nDES key after encrypting through RSA with receivers public key: ");
            ...
            if (xq == 'y' || xq == 'Y')
            {
                printf("\n\nENTER THE PRIVATE KEY(n,d) of the receiver: ");
                scanf("%d", &n);
                scanf("%d", &d);
                printf("\n\nDECRYPTING....\n");
